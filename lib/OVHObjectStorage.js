...
class OVHStorage {
  ...
  connection(callback, failback) {
    ...
    request(
      ...
      function(err, res, body) {
        ...
        var serviceCatalog = _.find(body.access.serviceCatalog, {
          type: "object-store"
        });
        var endpoint = _.find(serviceCatalog.endpoints, {
  ...
  container() {
    ...
    return {
      ...
      deleteAllObjects(container, callback, failback) {
        try {
          // lister les fichiers
          _this.container().list(
            container,
            function(data_list) {
              data_list =
                typeof data_list != "object"
                  ? JSON.parse(data_list)
                  : data_list;
              ...
              if (count <= 0) {
                ...
                for (var key in data_list) {
                  ...
                  _this.object().delete(
                    ...
                    function(e) {
                      throw new Error(
                        "Delete object error : " + file.name + " (" + e + ")"
                      );
                    }
                  );
                }
            ...
            function(e) {
              throw new Error(
                "List of objects error on container : " +
                  container +
                  " (" +
                  e +
                  ")"
      ...
      deleteForce(container, callback, failback) {
        try {
          ...
          _this.container().deleteAllObjects(
            container,
            function(result) {
              // delete container
              _this.container().delete(
                container,
                function() {
                  return callback(true);
                },
                function(e) {
                  throw new Error("Delete container error : " + e);
                }
              );
            },
            function(e) {
              throw new Error("Delete all files error : " + e);
            }
          );
        } catch (e) {
          return failback(e);
  ...
  object() {
    ...
    return {
      ...
      // set file
      set(data, path, callback, failback) {
        try {
